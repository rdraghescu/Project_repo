...
DATA_PATH = os.path.join(RAW_DATA_DIR, 'ga-sessions.csv') # Calea completa catre dataset
...
# Cream folderele necesare daca nu exista
...
logging.basicConfig(
    filename="C:/Users/User/Desktop/repos/Project_repo/errors_log.txt",
    level = logging.ERROR,
...
try:
    ...
    error_msg = f"Fisierul ga-sessions.csv nu a fost gasit in {RAW_DATA_DIR}"
...
# Salvam seturile procesate pentru reutilizare in alte scripturi
X_simple = df_clean[numeric_features].copy()
try:
    X_enhanced.to_csv(X_ENHANCED_PATH, index=False)
    X_simple.to_csv(X_SIMPLE_PATH, index=False)
    print(f"Seturi procesate salvate: {X_ENHANCED_PATH}")
    print(f"Seturi procesate salvate: {X_SIMPLE_PATH}")
except PermissionError:
    error_msg = f"Permisiuni insuficiente la salvarea fisierelor procesate in {PROCESSED_DATA_DIR}"
    logging.error(error_msg)
    raise ProjectErrorLog(error_msg)
except OSError as e:
    error_msg = f"Eroare la salvarea fisierelor procesate in {PROCESSED_DATA_DIR}: {e}"
    logging.error(error_msg)
    raise ProjectErrorLog(error_msg)
except Exception as e:
    error_msg = f"Eroare neasteptata la salvarea fisierelor procesate: {e}"
    logging.error(error_msg)
    raise ProjectErrorLog(error_msg)
...
# ===========================
# 2.0 TUNING HIPERPARAMETRI RANDOM FOREST ENHANCED (15 run-uri MLflow)
# ===========================
print("\n[2.0/5] Tuning hiperparametri Random Forest ENHANCED...")
print("   Se testează 4 combinații; rezultatele apar în MLflow UI.\n")

tuning_configs = [
    {
        'run_name': 'best_run',
        'params': {'n_estimators': 300, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },

    # {
    #     'run_name': 'tune_n_estimators_100',
    #     'params': {'n_estimators': 100, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 40},
    # },
    # {
    #     'run_name': 'tune_n_estimators_200',
    #     'params': {'n_estimators': 200, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 40},
    # },
    # {
    #     'run_name': 'tune_n_estimators_400',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 40},
    # },
    {
        'run_name': 'tune_n_estimators_100_max_depth_10',
        'params': {'n_estimators': 100, 'max_depth': 10, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },
    {
        'run_name': 'tune_n_estimators_200_max_depth_10',
        'params': {'n_estimators': 200, 'max_depth': 10, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },
       {
        'run_name': 'tune_n_estimators_300_max_depth_10',
        'params': {'n_estimators': 300, 'max_depth': 10, 'min_samples_split': 20, 'min_samples_leaf': 40},
    }, 
    {
        'run_name': 'best_run_n_estimators_400_max_depth_10',
        'params': {'n_estimators': 400, 'max_depth': 10, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },
    # {
    #     'run_name': 'tune_max_depth_10',
    #     'params': {'n_estimators': 300, 'max_depth': 10, 'min_samples_split': 20, 'min_samples_leaf': 40},
    # },
    {
        'run_name': 'tune_max_depth_20',
        'params': {'n_estimators': 400, 'max_depth': 20, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },
    {
        'run_name': 'tune_max_depth_50',
        'params': {'n_estimators': 400, 'max_depth': 50, 'min_samples_split': 20, 'min_samples_leaf': 40},
    },
    {
        'run_name': 'tune_min_samples_split_10',
        'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 10, 'min_samples_leaf': 40},
    },
    # {
    #     'run_name': 'tune_min_samples_split_30',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 30, 'min_samples_leaf': 40},
    # },
    # {
    #     'run_name': 'tune_min_samples_split_40',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 40, 'min_samples_leaf': 40},
    # },
    # {
    #     'run_name': 'tune_min_samples_leaf_10',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 10},
    # },
    # {
    #     'run_name': 'tune_min_samples_leaf_30',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 30},
    # },
    # {
    #     'run_name': 'tune_min_samples_leaf_60',
    #     'params': {'n_estimators': 400, 'max_depth': 30, 'min_samples_split': 20, 'min_samples_leaf': 60},
    # },
    # {
    #     'run_name': 'tune_weak_config_low',
    #     'params': {'n_estimators': 400, 'max_depth': 10, 'min_samples_split': 10, 'min_samples_leaf': 10},
    # },
    # {
    #     'run_name': 'tune_weak_config_high',
    #     'params': {'n_estimators': 400, 'max_depth': 50, 'min_samples_split': 40, 'min_samples_leaf': 60},
    # },
]

best_r2_tuning = -float('inf')
best_params_tuning = None
best_run_id_tuning = None
best_run_name_tuning = None

for idx, config in enumerate(tuning_configs, start=1):
    params = config['params']
    run_name = config['run_name']

    with mlflow.start_run(run_name=run_name) as tuning_run: # as tuning_run se refera la obiectul RandomForestRegressor
        ...
        y_tune_pred_train = rf_tune.predict(X_train_scaled)
        y_tune_pred = rf_tune.predict(X_test_scaled)

        r2_tune_train = r2_score(y_train, y_tune_pred_train)
        r2_tune = r2_score(y_test, y_tune_pred)
        mae_tune_train = mean_absolute_error(y_train, y_tune_pred_train)
        mae_tune = mean_absolute_error(y_test, y_tune_pred)
        rmse_tune_train = np.sqrt(mean_squared_error(y_train, y_tune_pred_train))
        rmse_tune = np.sqrt(mean_squared_error(y_test, y_tune_pred))

        mlflow.log_param("model_type", "RandomForestRegressor")
        mlflow.log_param("model_version", "Enhanced - Hyperparameter Tuning")
        mlflow.log_param("n_estimators", params['n_estimators'])
        mlflow.log_param("max_depth", params['max_depth'])
        mlflow.log_param("min_samples_split", params['min_samples_split'])
        mlflow.log_param("min_samples_leaf", params['min_samples_leaf'])
        mlflow.log_metric("r2_train", r2_tune_train)
        mlflow.log_metric("r2_test", r2_tune)
        mlflow.log_metric("rmse_train", rmse_tune_train)
        mlflow.log_metric("rmse_test", rmse_tune)
        mlflow.log_metric("mae_train", mae_tune_train)
        mlflow.log_metric("mae_test", mae_tune)
        mlflow.log_metric("training_time_seconds", tuning_time)
        mlflow.log_metric("overfitting_gap", r2_tune_train - r2_tune)
        mlflow.set_tag("run_type", "hyperparameter_tuning")
        mlflow.set_tag("tuning_index", str(idx))

        if r2_tune > best_r2_tuning:
            best_r2_tuning = r2_tune
            best_params_tuning = params.copy()
            best_run_id_tuning = tuning_run.info.run_id
            best_run_name_tuning = run_name

    print(f"   [{idx}/15] {run_name}: R² Test = {r2_tune:.4f}")

print("\n   Tuning finalizat. Cel mai bun model identificat:")
print(f"   Run name: {best_run_name_tuning}")
print(f"   Run ID: {best_run_id_tuning}")
print(f"   Parametri: {best_params_tuning}")
print(f"   R² Test: {best_r2_tuning:.4f}")
...
with mlflow.start_run(run_name="Random_Forest_ENHANCED_v2.0") as run:
    ...
    # ===========================
    # 3. ANTRENARE MODEL RANDOM FOREST (parametri optimi din tuning)
    # ===========================
    print("\n[3/5] Antrenare model Random Forest (ENHANCED)...")
    print("   Parametri optimi identificați prin tuning:")
    print(f"     - n_estimators: {best_params_tuning['n_estimators']} arbori")
    print(f"     - max_depth: {best_params_tuning['max_depth']} (adâncime optimă)")
    print(f"     - min_samples_split: {best_params_tuning['min_samples_split']}")
    print(f"     - min_samples_leaf: {best_params_tuning['min_samples_leaf']}")
    print(f"     - Sursă tuning: run '{best_run_name_tuning}' (ID: {best_run_id_tuning})\n")

    start_time = datetime.now()
    model = RandomForestRegressor(
        n_estimators=best_params_tuning['n_estimators'],        # Parametri optimi din tuning
        max_depth=best_params_tuning['max_depth'],              # Adâncime optimă pentru capturarea pattern-urilor complexe
        min_samples_split=best_params_tuning['min_samples_split'],    # Control pentru generalizare
        min_samples_leaf=best_params_tuning['min_samples_leaf'],     # Previne overfitting-ul
    ...
    mlflow.log_param("n_estimators", best_params_tuning['n_estimators'])
    mlflow.log_param("max_depth", best_params_tuning['max_depth'])
    mlflow.log_param("min_samples_split", best_params_tuning['min_samples_split'])
    mlflow.log_param("min_samples_leaf", best_params_tuning['min_samples_leaf'])
    mlflow.log_param("tuning_best_run_name", best_run_name_tuning)
    mlflow.log_param("tuning_best_run_id", best_run_id_tuning)
    mlflow.log_param("tuning_best_r2", best_r2_tuning)
    ...
    # Random Forest ENHANCED: R²=0.4671 (+1.41% vs SIMPLE) - CEL MAI BUN!
    # Linear Regression ENHANCED: R²=0.465 (+2.19% vs SIMPLE)
    # Polynomial ENHANCED: R²=0.271 (-40% vs SIMPLE, overfitting)
    # XGBoost ENHANCED: R²=0.4500 (+0.0044% vs SIMPLE)
    # LightGBM ENHANCED: R²=0.4419 (-0.0101% vs SIMPLE)
    models_comparison = {
        'Model': ['Random\nForest\n(ENHANCED)\n BEST', 'Linear\nRegression\n(ENHANCED)', 'Polynomial\nRegression\n(ENHANCED)', 'XGBoost\n(ENHANCED)', 'LightGBM\n(ENHANCED)'],
        'R²': [0.4671, 0.465, 0.271, 0.4500, 0.4419],
        'MAE (s)': [100.73, 111.48, 107.15, 103.14, 102.96],
        'RMSE (s)': [270.94, 271.43, 316.95, 275.24, 277.25],
        'Time (s)': [4.8, 0.02, 1.15, 6.2, 2.88]  # Timpul de antrenare pentru fiecare model
    }
    ...
    # Time
    bars4 = axes[3].bar(models_comparison['Model'], models_comparison['Time (s)'], 
                        color=['#2196F3', '#FF9800', '#4CAF50'], edgecolor='black', linewidth=1.5)
    axes[3].set_ylabel('Time (seconds)', fontsize=12, fontweight='bold')
    axes[3].set_title('Training Time Comparison\n(Mai scurt = Mai bun)', fontsize=14, fontweight='bold')
    axes[3].set_ylim([0, max(models_comparison['Time (s)']) * 1.2])
    ...
    # Salvam modelul LOCAL cu joblib în folderul models/
    ...
    try:
        ...
        #     pickle.dump(model, f) # pickle.dump() este o functie care serializaza un obiect Python (in acest caz, model-ul) intr-un format binar, care poate fi apoi salvat intr-un fisier. Acest lucru permite ulterior incarcarea obiectului exact asa cum a fost salvat, pastrand toate atributele si starea acestuia (de exemplu, parametrii, structura, etc.).
        joblib.dump(model, model_path, compress=3) # joblib.dump() este o functie care salveaza un obiect Python (in acest caz, model-ul) intr-un format binar, care poate fi apoi incarcat ulterior. Argumentul compress=3 comprima fisierul pentru a reduce dimensiunea sa, ceea ce poate fi util pentru a economisi spatiu de stocare si timp de transfer.   
    except PermissionError:
    ...
    # Logam modelul ca artifact in MLflow (pentru referinta)
    try:
    ...
    try:
        ...
        joblib.dump(scaler, scaler_path, compress=3) # joblib.dump() este o functie care salveaza un obiect Python (in acest caz, scaler-ul) intr-un format binar, care poate fi apoi incarcat ulterior. Argumentul compress=3 comprima fisierul pentru a reduce dimensiunea sa, ceea ce poate fi util pentru a economisi spatiu de stocare si timp de transfer.   
    except PermissionError:
        ...
        raise ProjectErrorLog(error_msg)
    try:
        mlflow.log_artifact(scaler_path, "preprocessors") # .log_artifact() este o functie generica pentru a salva orice fisier ca artifact in MLflow. In acest caz, salvam scaler-ul serializat cu joblib intr-un subfolder "preprocessors" din cadrul artifact-ului run-ului curent. Acest lucru ne permite sa organizam mai bine artifacts-urile si sa le accesam ulterior pentru reproducere sau analiza.
    except Exception as e:
    ...
    # Cream folderul predictions daca nu exista
    predictions_dir = os.path.join(PROJECT_ROOT, 'data', 'predictions') # os.path.join() este o functie care concateneaza doua sau mai multe string-uri in mod automat, in functie de sistemul de operare. In acest caz, concateneaza PROJECT_ROOT (care este calea absoluta catre root-ul proiectului) cu 'data' si 'predictions', formand o cale absoluta completa catre folderul 'predictions' din 'data'.
    os.makedirs(predictions_dir, exist_ok=True) # .makedirs() este o functie care creeaza un folder in sistemul de fisiere. Argumentul exist_ok=True face ca functia sa nu arunce o exceptie daca folderul exista deja, ci sa o suprascrie. 
    # Cream fisierul validation_results.csv in folderul predictions.
    # Acest fisier va contine 20% din datele reale de test preluate din data/raw/ga-sessions.csv.
    # El va contine coloanele: pageviews, visitNumber, hits, timeOnSite_REAL, timeOnSite_predicted, error_seconds, error_absolute, error_percent, model, validation_date, model_r2, model_mae, model_rmse.
...
print("   \u2713 R² Score: Cel mai mare (0.467)")
print("   \u2713 MAE: Cel mai mic (100.73s)")

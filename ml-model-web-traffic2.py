...
Vom testa si compara 5 algoritmi de regresie:
...
Features: pageviews, visitNumber, hits + features categorice + engineered features

Scopul este sa identificam cel mai bun model pentru predictia comportamentului utilizatorului.
Fiecare model este antrenat in 2 VERSIUNI:
   - Versiune SIMPLA: doar 3 features numerice originale
   - Versiune IMBUNATATITA: toate features-urile (numerice, categorice encodate, engineered)
"""
import logging
...
# Importuri pentru algoritmi avansati (Boosting)
try:
    import xgboost as xgb
    ...
    print("XGBoost nu este instalat. Instaleaza cu: pip install xgboost")
...
try:
    ...
    print(" LightGBM nu este instalat. Instalează cu: pip install lightgbm")
...
# ===========================
# SETARE FISIER LOG SI CREEARE CLASA PROPRIE PENTRU ERORI
# ===========================
print("\n[0/9] Setare fisier log si creare clasa proprie pentru erori...")

# Clasa proprie pentru erori si inregistrarea lor in fisierul error_log.log
# Intre error_log.txt si error_log.log nu exista diferenta semantica, ambele fiind fisiere de log.
# Mai des sunt folosite fisierele de tip log pentru a inregistra erorile, asa ca voi folosi si eu fisierul error_log.log.
...
logging.basicConfig(
    ...
    # Unde: %(asctime)s se refera la data si ora in care s-a intamplat eroarea, 
    # %(lineno)d se refera la numarul liniei in care s-a intamplat eroarea, 
    # %(filename)s se refera la numele fisierului in care s-a intamplat eroarea, 
    # iar %(message)s se refera la mesajul de eroare.
    datefmt='%Y-%m-%d %H:%M:%S' # se refera la formatul data si ora in care s-a intamplat eroarea.
    ) # Se configureaza logging-ul pentru a inregistra erorile in fisierul error_log.log, 
...
print("\n[1/9] Incarcarea datelor...")

try:
    data = pd.read_csv(DATA_PATH)
    print(f"✓ Date incarcate: {data.shape[0]:,} randuri, {data.shape[1]} coloane")

except FileNotFoundError: # Daca fisierul ga-sessions.csv nu este gasit, atunci se ridica o exceptie de tip ProjectErrorLog cu mesajul "Fisierul ga-sessions.csv nu a fost gasit!"
    error_msg ="Fisierul ga-sessions.csv din folderul raw nu a fost gasit!"
    logging.error(error_msg) # Se inregistreaza eroarea in fisierul error_log.log.
    raise ProjectErrorLog(error_msg) # Se ridica o exceptie de tip ProjectErrorLog cu mesajul "Fisierul ga-sessions.csv din folderul raw nu a fost gasit!"

except Exception as e: # Daca apare o alta exceptie, atunci se ridica o exceptie de tip ProjectErrorLog cu mesajul "Eroare la incarcarea datelor: {e}"
    error_msg = f"Eroare la incarcarea datelor: {e}"
    logging.error(error_msg)
    raise ProjectErrorLog(error_msg) # Se ridica o exceptie de tip ProjectErrorLog cu mesajul "Eroare la incarcarea datelor: {e}"
...
# ===========================
print("\n[2/9] Explorarea structurii datelor...")
...
# 3. CURATAREA DATELOR
# ===========================
print("\n[3/9] Curatarea si pregatirea datelor...")
...
# ===========================
print("\n[4/9] Definirea features si target...")
...
# | Este operatorul logic "sau" (OR) in Python.
...
if nan_mask_test.sum() > 0:
    ...
    X_simple_test = X_simple_test[~nan_mask_test] # ~nan_mask_test inverseaza masca booleana, astfel incat sa selecteze doar randurile care nu contin NaN-uri in X_simple_test. Aceasta operatie elimina efectiv randurile cu NaN-uri din setul de testare pentru versiunea simpla a modelului de regresie. De asemenea, y_test este filtrat in acelasi mod pentru a pastra consistenta intre features si variabila tinta, eliminand aceleasi randuri care contin NaN-uri in y_test.
    y_test = y_test[~nan_mask_test] # y_test este filtrat in acelasi mod pentru a pastra consistenta intre features si variabila tinta, eliminand aceleasi randuri care contin NaN-uri in y_test.
...
# ===========================
print("\n[5/9] Antrenarea modelelor de regresie...")
print("="*80)
...
"""
Pentru versiunea simple a modelului de regresie Random Forest nu am sa implementez acest cod, dar voi mentine codul pentru a fi folosit in viitor.
...
param_grid = {
    ...
    'min_samples_leaf': [5, 10, 15, 20, 25, 30, 40, 45]
}
...
# ===========================
# MODEL 4: XGBoost Regressor (Extreme Gradient Boosting)
# ===========================
print("\n" + "="*80)
print("MODEL 4: XGBoost Regressor (Extreme Gradient Boosting)")
print("="*80)
...
if XGBOOST_AVAILABLE:
    ...
    # Cream modelul XGBoost pentru versiunea simpla
    # XGBoost este un algoritm de gradient boosting extrem de eficient care construieste arbori secvential,
    # fiecare arbore incercand sa corecteze erorile arborilor anteriori
    xgb_simple_model = XGBRegressor(
        n_estimators=2500,          # foarte multi arbori pentru convergenta excelenta
        max_depth=3,                # arbori FOARTE superficiali pentru generalizare maxima
        learning_rate=0.03,         # learning rate foarte mic pentru invatare lenta si stabila
        subsample=0.95,             # aproape toate esantioanele
        colsample_bytree=1.0,       # toate features-urile (avem doar 3)
        min_child_weight=1,         # flexibilitate maxima
        gamma=0,                    # fara penalizare
        reg_alpha=0,                # ZERO regularizare L1
        reg_lambda=0,               # ZERO regularizare L2
        random_state=42,
        n_jobs=-1,
        verbosity=0
    )
    
    # Antrenam modelul XGBoost pe setul de antrenare simplu
    xgb_simple_model.fit(X_simple_train, y_train)
    
    # Predictii pe setul de testare
    y_xgb_simple_pred = xgb_simple_model.predict(X_simple_test)
    
    # Calculam timpul de antrenare
    xgb_simple_time = time.time() - start_time
    
    # Calculam metrici de evaluare
    xgb_simple_r2 = r2_score(y_test, y_xgb_simple_pred)
    xgb_simple_rmse = np.sqrt(mean_squared_error(y_test, y_xgb_simple_pred))
    xgb_simple_mae = mean_absolute_error(y_test, y_xgb_simple_pred)
    
    # Stocam rezultatele
    results_simple['XGBoost'] = {
        'model': xgb_simple_model,
        'predictions': y_xgb_simple_pred,
        'r2': xgb_simple_r2,
        'rmse': xgb_simple_rmse,
        'mae': xgb_simple_mae,
        'time': xgb_simple_time
    }
    
    print(f"      R² Score: {xgb_simple_r2:.4f}")
    print(f"      RMSE: {xgb_simple_rmse:,.2f} seconds")
    print(f"      MAE: {xgb_simple_mae:,.2f} seconds")
    print(f"      Training time: {xgb_simple_time:.4f}s")
    
    print(f"\n  Feature Importance (SIMPLA):")
    for feat, imp in zip(numeric_features, xgb_simple_model.feature_importances_):
        print(f"         {feat}: {imp:.4f}")
    ...
    # Cream modelul XGBoost pentru versiunea imbunatatita
    # Cu mai multe features, ajustam parametrii pentru a preveni overfitting-ul
    xgb_enhanced_model = XGBRegressor(
        n_estimators=3000,          # foarte multi arbori pentru capturarea pattern-urilor complexe
        max_depth=3,                # arbori superficiali pentru a preveni overfitting pe features multiple
        learning_rate=0.02,         # learning rate FOARTE mic pentru convergenta ultra-stabila
        subsample=0.95,             # aproape toate esantioanele pentru stabilitate
        colsample_bytree=0.9,       # 90% din features pentru diversitate
        min_child_weight=1,         # flexibilitate maxima
        gamma=0,                    # eliminam complet penalizarea
        reg_alpha=0,                # ZERO regularizare L1
        reg_lambda=0,               # ZERO regularizare L2 - lasam modelul liber
        random_state=42,
        n_jobs=-1,
        verbosity=0
    )
    ...
    # Antrenam modelul
    xgb_enhanced_model.fit(X_enhanced_train, y_train_enh)
    
    # Predictii
    y_xgb_enhanced_pred = xgb_enhanced_model.predict(X_enhanced_test)
    
    # Calculam timpul de antrenare
    xgb_enhanced_time = time.time() - start_time
    
    # Calculam metrici de evaluare
    xgb_enhanced_r2 = r2_score(y_test_enh, y_xgb_enhanced_pred)
    xgb_enhanced_rmse = np.sqrt(mean_squared_error(y_test_enh, y_xgb_enhanced_pred))
    xgb_enhanced_mae = mean_absolute_error(y_test_enh, y_xgb_enhanced_pred)
    
    # Stocam rezultatele
    results_enhanced['XGBoost'] = {
        'model': xgb_enhanced_model,
        'predictions': y_xgb_enhanced_pred,
        'r2': xgb_enhanced_r2,
        'rmse': xgb_enhanced_rmse,
        'mae': xgb_enhanced_mae,
        'time': xgb_enhanced_time
    }
    
    print(f"      R² Score: {xgb_enhanced_r2:.4f}")
    print(f"      RMSE: {xgb_enhanced_rmse:,.2f} seconds")
    print(f"      MAE: {xgb_enhanced_mae:,.2f} seconds")
    print(f"      Training time: {xgb_enhanced_time:.4f}s")
    
    print(f"\n Feature Importance (TOP 10 IMBUNATATITA):")
    feature_names_all = list(X_enhanced.columns)
    feature_importances = list(zip(feature_names_all, xgb_enhanced_model.feature_importances_))
    feature_importances_sorted = sorted(feature_importances, key=lambda x: x[1], reverse=True)
    for feat, imp in feature_importances_sorted[:10]:
        print(f"         {feat}: {imp:.4f}")
    
    # Comparatie
    improvement_r2 = xgb_enhanced_r2 - xgb_simple_r2
    improvement_pct = (improvement_r2 / xgb_simple_r2) * 100
    print(f"\n IMBUNATATIRE in cazul modelului XGBoost: R² {improvement_r2:+.4f} ({improvement_pct:+.1f}%)")
else:
    print("\n XGBoost nu este disponibil. Instaleaza cu: pip install xgboost")

# ===========================
# MODEL 5: LightGBM Regressor (Light Gradient Boosting Machine)
# ===========================
print("\n" + "="*80)
print("MODEL 5: LightGBM Regressor (Light Gradient Boosting Machine)")
print("="*80)
...
if LIGHTGBM_AVAILABLE:
    ...
    # Cream modelul LightGBM pentru versiunea simpla
    # LightGBM este un framework de gradient boosting bazat pe arbori extrem de rapid si eficient,
    # care foloseste tehnici precum leaf-wise tree growth si histogram-based algorithms
    # OPTIMIZARE: Strategia XGBoost - arbori superficiali, multi arbori, learning rate mic
    lgb_simple_model = LGBMRegressor(
        n_estimators=2500,          # foarte multi arbori pentru convergenta excelenta
        max_depth=3,                # arbori FOARTE superficiali pentru generalizare maxima
        # max_depth=10,               # LightGBM gestioneaza mai bine adancimea, putem permite ceva mai mult pentru a captura interactiuni
        learning_rate=0.03,         # learning rate foarte mic pentru invatare lenta si stabila
        num_leaves=7,               # num_leaves < 2^max_depth (2^3=8), folosim 7 pentru control
        # num_leaves=15,              # cu max_depth=10, putem permite mai multe leaves pentru a captura interactiuni
        subsample=0.95,             # aproape toate esantioanele
        colsample_bytree=1.0,       # toate features-urile (avem doar 3)
        min_child_samples=20,       # numarul minim de date necesare intr-o frunza
        reg_alpha=0,                # ZERO regularizare L1
        reg_lambda=0,               # ZERO regularizare L2
        random_state=42,
        n_jobs=-1,
        verbose=-1                  # elimina output-ul verbose
    )
    
    # Antrenam modelul LightGBM pe setul de antrenare simplu
    lgb_simple_model.fit(X_simple_train, y_train)
    
    # Predictii pe setul de testare
    y_lgb_simple_pred = lgb_simple_model.predict(X_simple_test)
    
    # Calculam timpul de antrenare
    lgb_simple_time = time.time() - start_time
    
    # Calculam metrici de evaluare
    lgb_simple_r2 = r2_score(y_test, y_lgb_simple_pred)
    lgb_simple_rmse = np.sqrt(mean_squared_error(y_test, y_lgb_simple_pred))
    lgb_simple_mae = mean_absolute_error(y_test, y_lgb_simple_pred)
    
    # Stocam rezultatele
    results_simple['LightGBM'] = {
        'model': lgb_simple_model,
        'predictions': y_lgb_simple_pred,
        'r2': lgb_simple_r2,
        'rmse': lgb_simple_rmse,
        'mae': lgb_simple_mae,
        'time': lgb_simple_time
    }
    
    print(f"      R² Score: {lgb_simple_r2:.4f}")
    print(f"      RMSE: {lgb_simple_rmse:,.2f} seconds")
    print(f"      MAE: {lgb_simple_mae:,.2f} seconds")
    print(f"      Training time: {lgb_simple_time:.4f}s")
    
    print(f"\n  Feature Importance (SIMPLA):")
    for feat, imp in zip(numeric_features, lgb_simple_model.feature_importances_):
        print(f"         {feat}: {imp:.4f}")
    ...
    # Cream modelul LightGBM pentru versiunea imbunatatita
    # OPTIMIZARE: Similar XGBoost - arbori superficiali, multe iteratii, zero regularizare
    lgb_enhanced_model = LGBMRegressor(
        n_estimators=3000,          # foarte multi arbori pentru capturarea pattern-urilor complexe
        max_depth=10,                # arbori superficiali pentru a preveni overfitting pe features multiple
        learning_rate=0.02,         # learning rate FOARTE mic pentru convergenta ultra-stabila
        num_leaves=10,               # control strict: 7 leaves pentru max_depth=3
        subsample=0.95,             # aproape toate esantioanele pentru stabilitate
        colsample_bytree=0.9,       # 90% din features pentru diversitate
        min_child_samples=20,       # flexibilitate pentru patterns complexe
        reg_alpha=0,                # ZERO regularizare L1 - lasam modelul liber
        reg_lambda=0,               # ZERO regularizare L2 - eliminam complet penalizarea
        random_state=42,
        n_jobs=-1,
        verbose=-1
    )
    ...
    # Antrenam modelul
    lgb_enhanced_model.fit(X_enhanced_train, y_train_enh)
    
    # Predictii
    y_lgb_enhanced_pred = lgb_enhanced_model.predict(X_enhanced_test)
    
    # Calculam timpul de antrenare
    lgb_enhanced_time = time.time() - start_time
    
    # Calculam metrici de evaluare
    lgb_enhanced_r2 = r2_score(y_test_enh, y_lgb_enhanced_pred)
    lgb_enhanced_rmse = np.sqrt(mean_squared_error(y_test_enh, y_lgb_enhanced_pred))
    lgb_enhanced_mae = mean_absolute_error(y_test_enh, y_lgb_enhanced_pred)
    
    # Stocam rezultatele
    results_enhanced['LightGBM'] = {
        'model': lgb_enhanced_model,
        'predictions': y_lgb_enhanced_pred,
        'r2': lgb_enhanced_r2,
        'rmse': lgb_enhanced_rmse,
        'mae': lgb_enhanced_mae,
        'time': lgb_enhanced_time
    }
    
    print(f"      R² Score: {lgb_enhanced_r2:.4f}")
    print(f"      RMSE: {lgb_enhanced_rmse:,.2f} seconds")
    print(f"      MAE: {lgb_enhanced_mae:,.2f} seconds")
    print(f"      Training time: {lgb_enhanced_time:.4f}s")
    
    print(f"\n Feature Importance (TOP 10 IMBUNATATITA):")
    feature_names_all = list(X_enhanced.columns)
    feature_importances = list(zip(feature_names_all, lgb_enhanced_model.feature_importances_))
    feature_importances_sorted = sorted(feature_importances, key=lambda x: x[1], reverse=True)
    for feat, imp in feature_importances_sorted[:10]:
        print(f"         {feat}: {imp:.4f}")
    
    # Comparatie
    improvement_r2 = lgb_enhanced_r2 - lgb_simple_r2
    improvement_pct = (improvement_r2 / lgb_simple_r2) * 100
    print(f"\n IMBUNATATIRE in cazul modelului LightGBM: R² {improvement_r2:+.4f} ({improvement_pct:+.1f}%)")
else:
    print("\n LightGBM nu este disponibil. Instalează cu: pip install lightgbm")

# ===========================
# 8. COMPARAREA REZULTATELOR (SIMPLA vs IMBUNATATITA)
# ===========================
print("\n" + "="*80)
print("[8/9] COMPARAREA PERFORMANTEI MODELELOR")
print("="*80)

# Cream DataFrames pentru comparatie
comparison_simple = pd.DataFrame({
    'Model': list(results_simple.keys()), # list[Any] reprezinta o lista de orice tip de obiect. De exemplu, daca avem un dictionar results_simple cu cheile 'Polynomial Regression' si 'Random Forest', atunci list(results_simple.keys()) va returna ['Polynomial Regression', 'Random Forest'].
    'R² Score': [results_simple[m]['r2'] for m in results_simple.keys()], # results_simple[m]['r2'] acceseaza valoarea R² pentru fiecare model, stocata in dictionarul results_simple, unde m reprezinta numele modelului (ex: 'Polynomial Regression', 'Random Forest'). Acest loop va extrage valorile R² pentru toate modelele din versiunea simpla si le va include in coloana 'R² Score' a DataFrame-ului comparison_simple, oferind astfel o perspectiva clara asupra performantei fiecarui model in versiunea sa simpla.
...
# ===========================
# 9. VIZUALIZARI (COMPARATIE SIMPLA vs IMBUNATATITA)
# ===========================
print("\n[9/9] Generarea vizualizarilor...")
...
# Subplot 7: Distributia erorilor - Simpla
...
try:
    ...
    logging.error(f"Eroare la salvarea graficelor: {e}")
    raise ProjectErrorLog(f"Eroare la salvarea graficelor: {e}")
...
if best_enhanced_r2 >= 0.7:
    print(f"EXCELENT! Modelul ofera predictii foarte bune (R² ≥ 0.7)")
elif best_enhanced_r2 >= 0.5:
    print(f"BUN! Modelul ofera predictii acceptabile (R² ≥ 0.5)")
elif best_enhanced_r2 >= 0.3:
    print(f"MODERAT. Modelul ofera predictii limitate (R² ≥ 0.3)")
else:
    print(f"SLAB. Modelul nu ofera predictii foarte utile (R² < 0.3)")
...
print(f"\nRECOMANDARI FINALE:")
print(f"   1. Folositi modelul {best_enhanced_name} cu features imbunatatite")
print(f"   2. Monitorizati performanta pe date noi (R² > {best_enhanced_r2:.2f})")
print(f"   3. Continuati feature engineering:")
print(f"      - Adaugati mai multe features categorice daca sunt disponibile")
print(f"      - Testati interactiuni intre features (pageviews × channelGrouping, etc.)")
print(f"      - Explorati agregari temporale (zi saptamana, ora zilei)")
print(f"   4. Testati hyperparameter tuning pentru Random Forest:")
print(f"      - n_estimators: 200-500")
print(f"      - max_depth: 20-30")
print(f"      - min_samples_split: 5-20")
print(f"   5. Considerati modele ensemble (combinatie Linear + Random Forest)")
...
print("\n Proiectul este gata pentru implementare in productie!")
print("="*80)


# De vazut daca se poate introduce o verificare a:
# A) Calitatii datelor din setul de date folosit, asta insemnand sa verific urmatoarele:
# 1. sa verific daca distributia datelor este normala (stats.normaltest())
# 2. sa verific daca exista outliers (stats.zscore() sau folosind codul de mai jos:
...
#     for value in data:
#         z = (value - mean) / std
#         assert abs(z) < threshold, "Outlier detected"

# detect_outliers(df['feature_1'])

# )

# B) Teste de convergenta pentru modelele de regresie.
# ex: 
# def test_model_convergence(training_loss):
#     assert training_loss[-1] < 0.1, "Model did not converge properly"

# training_loss = model.train(data)
# test_model_convergence(training_loss)

# C) Teste de performanta pentru modelele de regresie. 
# Exemple:
# - validation metrics - eg accuracy, precision, recall, f1-score; 
# - cross-validation techniques - eg k-fold cross-validation, leave-one-out cross-validation.

# D) Teste in proiecte de Machine Learning:
# - integration testing (end-to-end pipeline testing);

# E) Teste pentru Concept drift detection (monitor and test for concept drift):
# - data drift and concept drift analysis;
# Concept drift refers to the phenomenon where the statistical properties of the target variable change over time, causing a machine learning model’s predictions to become less accurate.
# Data drift occurs when the input data distribution changes over time, impacting a machine learning model’s performance. 
# Unlike concept drift, which refers to shifts in the target variable, data drift specifically concerns the features or input variables that 
# the model relies on.

# F) teste pentru a vdea daca modelul este corect(fairness testing) sau daca este afectat de idei raciste, sexiste, etc. (bias testing):
# Exemplu:
# from sklearn.metrics import confusion_matrix

# def test_model_fairness(y_true, y_pred, sensitive_attribute):
#     # Generate a confusion matrix for each group
#     for group in set(sensitive_attribute):
#         group_indices = [i for i, val in enumerate(sensitive_attribute) if val == group]
#         cm = confusion_matrix([y_true[i] for i in group_indices], [y_pred[i] for i in group_indices])

#     # Analyze false positives/negatives or other metrics for fairness
#     disparity = cm[0][1] / sum(cm[0]) - cm[1][1] / sum(cm[1])
#     assert abs(disparity) < 0.1, f"Model is biased towards group {group}"


# test_model_fairness(
#     y_test, model.predict(X_test), sensitive_feature='gender'
#  )
# In this example, the fairness check ensures that model performance 
# remains consistent across different demographic groups, reducing bias and promoting fairness.